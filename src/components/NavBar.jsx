      ...
      {navContent.map((navItem, i) => (
        ...
        >
          <Link
            ...
            key={i}
            href={navItem.link}
            className={`text-white px-4 flex items-center gap-2 py-1 text-sm md:text-lg 2xl:text-xl border-[1px] border-slate-400 rounded-lg  hover:border-white hover:ring-1 bg-black ring-amber-400 ${navItem.link===pathname ? 'ring-1 ring-red-400' : ''}`}
          >
