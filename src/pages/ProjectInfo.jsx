  ...
  if (!projectData || projectData.released === false) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black">
        <h2 className="text-3xl text-center font-bold mb-4 text-amber-50">
          Project not found or Yet to be released
        </h2>
        <button
  ...
  return (
    ...
    >
      <Link
        ...
        className="inline-flex text-lg items-center text-white hover:text-blue-600 mb-6"
      >
        <FaArrowLeft className="mr-2" /> Back
      </Link>
      <motion.h1
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 items-start">
        ...
        <div className="md:col-span-2 flex flex-col gap-5">
          ...
          <p className="italic text-white text-base md:text-xl">
      ...
      {projectData.managingTeam && projectData.managingTeam.length > 0 && (
        <div className="mt-12">
          <h3 className="text-2xl md:text-3xl font-semibold mb-8 text-white">
            Managing Team
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-7">
            {projectData.managingTeam.map((member, idx) => (
              <motion.div
                key={member.name || idx}
                whileHover={{
                  scale: 1.07,
